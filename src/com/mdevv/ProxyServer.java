...
/**
 * ProxyServer is a main class that contains the core logic of the server.
 * It also contains static Main method which is an entry point to the program.
 */
public class ProxyServer {

    /**
     * Start a program by reding configuration file and creating ProxyServer class based on it.
     * If arguments or configuration are invalid program closes with exit code 1.
     *
    ...
    public void listen() {
        try {
            ...
            while (true) {
                executor.submit(new RequestHandler(serverSocket.accept(), configuration.getFilteredWords(), cacheManager));
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
